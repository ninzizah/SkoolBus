...
import { MapPin, Users, CheckCircle, XCircle, AlertTriangle, Clock, ListChecks, Bus } from 'lucide-react';
...
export default function DriverDashboardPage() {
  ...
  const allChildrenProcessed = assignedRoute.stops.every(stop => 
    stop.children.every(child => child.status === 'Picked Up' || child.status === 'Dropped Off')
  );
    ...
    <div className="container mx-auto p-4 md:p-8">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        <div className="md:col-span-2">
          <Card className="shadow-lg">
            ...
            <CardContent>
                    ...
                    <AccordionItem value={stop.id} key={stop.id}>
                      <AccordionTrigger className="hover:no-underline text-lg font-medium py-4 px-2 rounded-md data-[state=open]:bg-muted/50">
                        <div className="flex items-center gap-3">
                          ...
                          <span>Stop {stopIndex + 1}: {stop.name}</span>
                          <Badge variant="outline" className="ml-auto">{stop.time}</Badge>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent className="pt-4 pb-2 px-2 space-y-4">
                        {stop.children.length === 0 && stop.name.toLowerCase().includes('school') && (
                           <p className="text-muted-foreground">This is a drop-off location. Ensure all picked-up children are marked as dropped.</p>
                        )}
                        {stop.children.length === 0 && !stop.name.toLowerCase().includes('school') && (
                           <p className="text-muted-foreground">No children assigned for pickup/drop-off at this stop.</p>
                        )}
                        {stop.children.map((child) => (
                          <div key={child.id} className="flex items-center justify-between p-3 border rounded-md shadow-sm bg-background hover:bg-muted/30 transition-colors">
                            <div className="flex items-center">
                              ...
                              <div>
                                ...
                                <Badge variant={
                                  ...
                                  child.status === 'Picked Up' ? 'bg-green-500 hover:bg-green-600' :
                                  child.status === 'Dropped Off' ? 'bg-blue-500 hover:bg-blue-600' : ''
                                }>{child.status}</Badge>
                              </div>
                            </div>
              ...
              {allChildrenProcessed && assignedRoute.stops.length > 0 && (
                 <div className="mt-6 p-4 bg-green-100 text-green-700 rounded-md text-center font-medium">
                  All children processed for this route!
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <div className="md:col-span-1 space-y-6">
          <Card className="shadow-lg">
            <CardHeader>
              <CardTitle className="font-headline flex items-center"> <Bus className="mr-2 h-5 w-5 text-primary" /> Route Overview</CardTitle>
            </CardHeader>
            <CardContent>
              <p><strong>Route:</strong> {assignedRoute.routeName}</p>
              <p><strong>Total Stops:</strong> {assignedRoute.stops.length}</p>
              <p><strong>Status:</strong> {allChildrenProcessed ? "Completed" : "In Progress"}</p>
              <div className="mt-4">
                <Image src="https://placehold.co/600x300.png" alt="Route map simplified" width={600} height={300} className="rounded-md" data-ai-hint="route map icon"/>
              </div>
            </CardContent>
          </Card>
          <Card className="shadow-lg">
